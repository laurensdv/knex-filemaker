...
function Formatter_VDB_JDBC() {
  this.client = client;
  this.paramCount = 0;
  Formatter.apply(this, arguments);
}
inherits(Formatter_VDB_JDBC, Formatter);

Formatter_VDB_JDBC.prototype.operators = [
  '=', '<>', '>', '>=', '<', '<=',
  'LIKE', 'NOT LIKE', 'IS NULL', 'IS NOT NULL', 'BETWEEN', 'IN', 'NOT IN', 'EXISTS', 'ANY', 'ALL'
];

// Wraps a value (column, tableName) with the correct ticks.
Formatter_VDB_JDBC.prototype.wrapValue = function(value) {
...
Formatter_VDB_JDBC.prototype._wrap = wrapperMemo;

// Assign the formatter to the the client.
client.Formatter = Formatter_VDB_JDBC;
